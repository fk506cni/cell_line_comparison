...
barplot.enrichResult <- function(height, x="Count", colorBy='pvalue', showCategory=5, font.size=12, title="",low_col="blue",high_col="red",ylm=c(0,20), ...) {
    ...
    p <- p +
        ...
        ylab(paste("-ln(",x,")",sep = ""))+coord_flip() + theme_dose(font.size)
...
# david_a <- S3Class()
# david_a <- data.table::fread("./david_Agenes.csv")
david_a$Description <-sapply(david_a$Description,newliner)
...
ggsave(filename = "david_A_mod.tiff",plot = a_result,dpi = 300,width = unit(9, "cm"),height = unit(5,"cm"))
...
ggsave(filename = "david_B_mod.tiff",plot = b_result,dpi = 300,width = unit(9, "cm"),height = unit(5,"cm"))

...
```{r setup}
knitr::opts_chunk$set(echo = TRUE, cache =F,cache.lazy = FALSE, echo=TRUE)
...
df <- df %>%
  ...
  mutate(isFBlike = pca_fb > control_point) %>%
  # mutate(isFBlike = F) %>%
...
```{r}
df_0liv <- df %>%
  dplyr::filter(origin != "Liver") %>%
  dplyr::filter(origin != "Small intestine") %>%
  dplyr::filter(origin != "Salivary gland") %>%
  dplyr::filter(origin != "Pleura")

vioScat_0liv <- ggVS(df_0liv)
vioScat_0liv

ggsave2(vioScat_0liv, wid = 18, hei = 18)
```
...
```{r}
cor_FBs <- cors_suspFB[cors_suspFB$origin %in% c("LARGE_INTESTINE", "BREAST", "SKIN","OESOPHAGUS","UPPER_AERODIGESTIVE_TRACT","LIVER","URINARY_TRACT","PLEURA","LUNG","BONE"),] %>%
  mutate(., origin_label = factor(.$origin,
                                  levels = c("LARGE_INTESTINE", "BREAST",  "SKIN", "OESOPHAGUS", 
                                             "UPPER_AERODIGESTIVE_TRACT", "LIVER", "URINARY_TRACT","PLEURA","LUNG","BONE"),
                                  labels = c("LARGE_INT", "BREAST",  "SKIN", "OESOPH", 
                                             "UA_T", "LIVER", "UR_T","PLEURA","LUNG","BONE")))
...
cor_FBselected <- cor_FBs %>%
  mutate(FBsusp = ifelse(pca_fb > control_point, "FBlike","NOT"))
...
dendfunc <- function(data,meta_data,
    ...
    scale_y_reverse(expand=c(0,0),limits = c(max(range(segment(ddata)$y))+0.1,0),breaks=axisX)+
...
gR
    ...
    labs_table_mg <- merge(labs_table, ccle_annot, by.x="label", by.y="CCLE.name", all.x=T, sort=F) %>%
      .[rev(1:nrow(.)),] %>%
      as.data.frame() %>%
      dplyr::rename(Type = FBsusp)  %>%
      dplyr::select(one_of(c("label","Cell.line.primary.name", "Type", "ID")))
    
    res_list$labs_table <- labs_table_mg
    return(res_list)
}
...
tg2ggsave <- function(tag, data4clst, meta4clst,clst_method = "average",wid = 9, hei =9, seg=0.1){
  ...
  d <- dendfunc(data = data, meta_data = meta_data,clst_method = clst_method,legend_label = "Type",main = main,line_size = 0.3,seg=seg, tag= tag)
  
  file_name <- paste(tag,"_",clst_method, ".tiff", sep = "",collapse = "")
  unit <- "cm"
  ggsave(filename = file_name,plot = d$gar,device = "tiff",width = wid, height = hei,dpi = 300,units = "cm")
  
  tablex <- paste(tag,"_",clst_method, ".xlsx", sep = "",collapse = "")
  
  labs_table <- d$labs_table %>%
    mutate(origin = tag) %>%
    select(origin,everything())
  openxlsx::write.xlsx(x=labs_table,
                       file = tablex)
...
tg2ggsave(tag = "BONE",data4clst = data4clst, meta4clst = meta4clst)
...
# li_labs_table <- 
tg2ggsave(tag = "LARGE_INTESTINE", data4clst = data4clst, meta4clst = meta4clst, clst_method = "ward.D2", seg = 0.2)
tg2ggsave(tag = "BONE", data4clst = data4clst, meta4clst = meta4clst, clst_method = "ward.D2",
          seg = 0.2)
# sk__labs_table <- 
tg2ggsave(tag = "SKIN",data4clst = data4clst, meta4clst = meta4clst, clst_method = "ward.D2", seg = 0.2)
tg2ggsave(tag = "BREAST",data4clst = data4clst, meta4clst = meta4clst, clst_method = "ward.D2", seg = 0.2)
tg2ggsave(tag = "PLEURA",data4clst = data4clst, meta4clst = meta4clst,clst_method = "ward.D2", seg = 0.2)
tg2ggsave(tag = "LUNG",data4clst = data4clst, meta4clst = meta4clst,clst_method = "ward.D2", seg = 0.2)
```

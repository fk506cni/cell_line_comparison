...
```{r}
getUniqHiGT <- function(gtlist){
  require("ggplot2")
  require(gtable)
  require(gridExtra)
  
  getH <- function(list){
    hl <- list$heights[2:10]
    return(list(hl))
  }
  ...
  h_list <- sapply(gtlist,getH)
  
  #h_list
  #class(h_list)
  h_maxs <- do.call(grid::unit.pmax, h_list)
  
  #h_maxs
  #return(h_maxs)
  
  for(i in 1:length(gtlist)){
    gtlist[[i]]$heights[2:10] <- as.list(h_maxs)
  }
...
#gtlist <- tmp[2:7]
#gtlist
#tmp2 <- getUniqHiGT(list(tmp$gL_1, tmp$gL_2))

# gp1$heights
# grid::unit.pmax(gp1$heights[2:10], gp2$heights[2:10],gp2$heights[2:10])
# tmp <- as.list(grid::unit.pmax(gp1$heights[2:10], gp2$heights[2:10],gp2$heights[2:10]))[[1]]
# class(tmp)
# tmp <- grid::unit.pmax(gp1$heights, gp2$heights)
# tmp
# tmp <- grid::unit.pmax(do.call(list(gp1$heights, gp2$heights)))
# tmp <- do.call(grid::unit.pmax,list(gp1$heights, gp2$heights))
# tmp
# 
# list(gp1$heights, gp2$heights)
...
labels <- c("BREAST", "LARGE_INTESTINE", "LUNG", "SKIN")
tag <- "SKIN"
i <- 4
data <- data4clst
meta_data <- meta4clst
legend_label <- "Expression\nType"
legend_lab_class <- c("FB_like", "NOT")
...
multidend <- function(
  ...
  seg= 0.3
    ...
    ##loop generation of dend
    label_length <- length(labels)
    dend_list <- list()
    
    dend_list$blank <- blank_grid
    ...
    for(i in 1:label_length){
      ...
      annot_str <- tag %>%
        ...
        tolower() %>%
        Hmisc::capitalize()
      metadata_i <- meta_data %>%
        dplyr::filter(origin == tag)
      
      data_i <- data %>%
        .[,colnames(.) %in% metadata_i$rowname]
      
      dend <- as.dist(1 - cor(data_i, method = dist_method)) %>% hclust(method = clst_method) %>% as.dendrogram
      ddata <- dendro_data(dend)
      labs <- label(ddata)
      ...
      dplyr::select(rowname, FBsusp)
      labs_table <- merge(x = labs,y = metadata_i,by.x="label", by.y ="rowname", sort = F)
    ...
    if (is.null(col_palette)){
      ...
      scale_x_continuous(expand = c(0,0),limits = range(segment(ddata)$x)+c(-1,1))+
      scale_y_continuous(expand = c(0,0))+
      scale_fill_manual(values = col_palette, labels = legend_lab_class)+
      labs(fill=legend_label)+
      coord_flip()+
      theme(axis.line.y=element_blank(),
          axis.ticks.y=element_blank(),
      ...
      gR
      gp2 <- ggplotGrob(gR)
      id.legend <- grep("guide", gp2$layout$name)
      legend_gtable <- gp2[["grobs"]][[id.legend]]
      
      gp2 <- ggplotGrob(gR+theme(legend.position="none"))
      
      dendlist_name <- paste("gR_",as.character(i), sep="",collapse = "")
      dend_list[[dendlist_name]] <- gp2
      
            gL <- ggplot(data=segment(ddata))+
        geom_segment(aes(x=x, y=y, xend=xend, yend=yend),size=line_size)+
        annotate(geom = "text", x = Inf, y=Inf, label=annot_str, hjust=0, vjust=1, size =7)+
        scale_y_reverse(expand=c(0,0),limits = c(max(range(segment(ddata)$y)),0),breaks=axisX)+
        scale_x_continuous(expand = c(0,0),limits = range(segment(ddata)$x)+c(-1,1))+
        coord_flip()+
        theme(axis.line.y=element_blank(),
      ...
      if(2 < i){
        ...
        gL <- gL +theme(axis.title.x = element_blank())
      }
      
      gp1 <- ggplotGrob(gL)
      
      dendlist_name <- paste("gL_",as.character(i), sep="",collapse = "")
      dend_list[[dendlist_name]] <- gp1
    }
    
    dend_list <- getUniqHiGT(dend_list)
    
    dend_list$legend <- legend_gtable
    
    lmat <- rbind(c(1,1,1,1,1,1,1),
                  c(1,3,2,1,5,4,10),
                  c(1,7,6,1,9,8,10),
                  c(1,1,1,1,1,1,1))
    lwid <- c(0.01, 2, 0.5, 0.15,2, 0.5, 1.5)
    lhei <- c(0.01,1,1,0.01)
    
    args <- c(dend_list, list(layout_matrix = lmat, heights=lhei, widths=lwid))
    
     gar <- do.call(grid.arrange, args)
     grid.draw(gar)
     return(gar)
...
dend4 <- multidend(data = data4clst, meta_data = meta4clst,labels = labels, legend_label = "Expression\nType",main = "")

ggsave2(plot = dend4,wid = 18,hei = 18)

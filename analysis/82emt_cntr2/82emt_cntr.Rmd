...
##file_preparation
```{r untarfiles}
untar(tarfile = "./GSE28798_RAW.tar")
for (i in list.files(pattern = ".gz")){
    gunzip(i)
}


list.files(pattern = "promR.CEL")
file.remove(list.files(pattern = "promR.CEL"))
...
label <- c("HepG2_siControl_0h","HepG2_siControl_TGF-beta_1p5h","HepG2_siControl_TGF-beta_24h","HepG2_siHNF4a_0h","HepG2_siHNF4a_TGF-beta_1p5h","HepG2_siHNF4a_TGF-beta_24h")
...
heatmap.2(d,cexRow = 0.3,cexCol = 0.5,trace = "none",dendrogram ="both" ,Rowv = T, Colv = T,scale = "none",srtRow = 45,srtCol = 45,col =bluered(255),breaks = pairs.breaks,symkey = F, RowSideColors = d_col,main = "GSE28798")
...
colslider <- c("pink","red", "darkred","lavender","violet","purple")
...
```{r}
font_size <- 14
lmat <- rbind(c(2,8,8,1,1,6),
        c(1,8,8,1,1,1),
        c(1,1,9,1,1,1),
        c(1,1,4,1,1,1),
        c(1,10,3,5,7,1)
        )
lwid <- c(0.01, 0.05, 1.3, 0.05,0.2,0.01)
lhei <- c(0.12,0.4,0.8,0.1,1.4)
